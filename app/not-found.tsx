  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (pathname.startsWith('/blog/')) {
        ...
        if (rawSlug) {
          const matchResult = findBlogPostOrMatch(rawSlug);
          if (matchResult.post) {
            window.location.replace(`/blog/${encodeURIComponent(matchResult.post.slug)}`);
          }
        }
      }
    }
    ...
    <div className="flex flex-col min-h-screen bg-brand-gray font-sans text-brand-secondary">
      ...
      <main className="flex-grow pt-24 pb-32 flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-lg text-gray-600 max-w-lg mb-10 leading-relaxed">
          يبدو أن الصفحة التي تبحث عنها غير موجودة أو تم نقلها. جاري تحويلك إلى المقال ذي الصلة أو يمكنك العودة للصفحة الرئيسية.
        </p>
        <Link 
          href="/" 
